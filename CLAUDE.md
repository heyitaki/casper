...
- **Maintain `docs/casper-fork.md`.** It is a concise index — one line per patch linking to a detail doc in `docs/casper-fork/` (files touched, summary, deletion condition). Per-file merge guidance lives in `docs/casper-fork/merge-conflict-notes.md`; retired patches in `docs/casper-fork/retired.md`. Update both the index line and the detail doc whenever a patch lands, retires, or conflicts.
