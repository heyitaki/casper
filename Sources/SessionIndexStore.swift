...
final class SessionIndexStore: ObservableObject {
    ...
    /// Path to `rg` (ripgrep). nil when not found — callers fall back to the
    /// Foundation substring scan. See `RipgrepLocator` for resolution order.
    nonisolated private static let cachedRipgrepPath: String? = RipgrepLocator.executableURL?.path
