...
struct FileExplorerPanelView: NSViewRepresentable {
    ...
    func updateNSView(_ container: FileExplorerContainerView, context: Context) {
        ...
        context.coordinator.onContainerChange?(container)
        // CASPER: sync externally-driven Find resets and search option buttons before
        // updatePresentation so mode switches that expose the Find tab start with
        // consistent field/results state. Delete if upstream adds first-class find
        // state management on FileExplorerStore.
