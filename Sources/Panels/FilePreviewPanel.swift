...
enum FilePreviewKindResolver {
    ...
    // CASPER: FileExplorerView calls isExplicitTextFile(filename:ext:) for Finder-row
    // icon substitution. Keep these two shims as long as FileExplorerView references
    // them; delete if upstream adds a public textExtensions membership check or
    // exposes a dedicated icon-resolution helper.
