...
class TerminalController {
    ...
    // CASPER: off-main dedup for report_ports bursts (deploys binding many
    // listening ports). The package SocketFastPathState only dedups shell
    // activity; delete if upstream adds ports dedup there.
    private static let casperPortsDedup = CasperPortsPublishDedup()

    private final class CasperPortsPublishDedup: @unchecked Sendable {
        private struct Key: Hashable {
            let workspaceId: UUID
            let panelId: UUID
        }

        private let queue = DispatchQueue(label: "com.cmux.casper-ports-dedup")
        private var lastReportedPorts: [Key: [Int]] = [:]
        private let maxTrackedPorts = 4096

        /// Caller passes the already-normalized (sorted + uniqued) port list so
        /// equality comparison is O(n) without re-allocating Sets per call.
        func shouldPublishPorts(workspaceId: UUID, panelId: UUID, normalizedPorts: [Int]) -> Bool {
            let key = Key(workspaceId: workspaceId, panelId: panelId)
            return queue.sync {
    ...
    private func processV2Command(_ jsonLine: String) -> String {
        ...
        case "surface.set_title":
            return v2Result(id: id, self.v2SurfaceSetTitle(params: params))
    ...
    private func v2SurfaceSetTitle(params: [String: Any]) -> V2CallResult {
        guard let tabManager = v2ResolveTabManager(params: params) else {
            return .err(code: "unavailable", message: "TabManager not available", data: nil)
        }
        ...
        var result: V2CallResult = .err(code: "internal_error", message: "Failed to set title", data: nil)
        v2MainSync {
            guard let ws = v2ResolveWorkspace(params: params, tabManager: tabManager) else {
                result = .err(code: "not_found", message: "Workspace not found", data: nil)
                return
            ...
            ws.updatePanelTitle(panelId: surfaceId, title: title)
            let windowId = v2ResolveWindowId(tabManager: tabManager)
            result = .ok([
                "workspace_id": ws.id.uuidString,
                "workspace_ref": v2Ref(kind: .workspace, uuid: ws.id),
    ...
    private func reportPorts(_ args: String) -> String {
        ...
        if let scope = Self.explicitSocketScope(options: parsed.options) {
            guard Self.casperPortsDedup.shouldPublishPorts(
