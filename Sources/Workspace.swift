...
final class Workspace: Identifiable, ObservableObject {
    ...
    // CASPER: thin shim so TerminalController.appendLog callers don't need
    // to know about log-entry trimming; delete if upstream inlines this at
    // every call site in TerminalController (or if TerminalController.swift
    // resolves to upstream's inline pattern).
    func appendLog(_ entry: SidebarLogEntry) {
        let trimmed = entry.message.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return }
        let normalizedEntry = SidebarLogEntry(
            message: trimmed,
            level: entry.level,
            source: entry.source,
            timestamp: entry.timestamp
        )
        logEntries.append(normalizedEntry)
    ...
    // indicator ("3 blue dots"). statusEntries now lives directly on
    // Workspace (sub-store dropped), so subscribe to $statusEntries here.
    // The non-compact TabItemView subscribes per-row via sidebarObservationPublisher;
    // the compact path (VerticalTabsSidebar) uses .equatable() rows and
    // needs this separate signal. Consumed by CasperSidebarActivityRefresher.
    // Delete with the activity-state patch if upstream surfaces agent state
    ...
    init(
        ...
        let config = BonsplitConfiguration(
            ...
            // CASPER: .multipleTabs hides the tab bar when only one tab is open,
            // matching Casper's single-session default UX. Delete if upstream
            // adopts this as the default or adds a user-facing setting.
    ...
    func pruneSurfaceMetadata(validSurfaceIds: Set<UUID>) {
        ...
        // CASPER: panelTitles and panelPullRequests were pruned via sidebarMetadataStore.pruneToValidSurfaces
        // (sub-store pattern); now inlined here after sub-store was dropped in favour of direct @Published
        // properties. Delete this comment if upstream adopts direct properties for these too.
