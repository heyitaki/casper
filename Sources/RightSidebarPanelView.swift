...
struct RightSidebarPanelView: View {
    ...
    private var modeBar: some View {
        ...
        // CASPER: Numbered Ctrl-N shortcuts only route when the right sidebar (or a
        // Dock-surface terminal) is focused, so their hints stay gated.
        // Tabs whose `usesUngatedShortcutHintMonitor` is true (Find via ⌘⇧F today)
        // drive their hint visibility from the ungated focus monitor. Delete if
        // upstream adds first-class ungated hint routing per tab.
