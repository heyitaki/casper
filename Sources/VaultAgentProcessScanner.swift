...
// Delete if upstream replaces this scanner with a libproc-based path or
// caches argv at the processArgumentsProvider call site.
    ...
    static func processDetectedSnapshots(
        ...
        for process in scopedProcesses {
            guard let workspaceId = process.cmuxWorkspaceID,
                  ...
                  // CASPER: argv is immutable post-exec; cache by (pid, name, ppid) so
                  // repeated scans skip sysctl(KERN_PROCARGS2). Delete if upstream caches
                  // at the processArgumentsProvider call site.
                  let processArguments = VaultAgentArgvCache.parsedArguments(
                      for: process.pid,
                      name: process.name,
                      parentPID: process.parentPID,
                      parse: processArgumentsProvider
                  ) else {
                continue
            }
