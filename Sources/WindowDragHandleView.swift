...
enum MinimalModeSidebarTitlebarControlsMetrics {
    ...
    // CASPER: hostWidth is derived from visibleSlots.count so Casper's narrower
    // cluster (newTab only) doesn't reserve unused space. Stock builds compute
    // the same 164 as the previous hardcoded value. The trailing reserve (36 pt)
    // covers shortcut-hint pills and shadow margin. Delete the CasperReloadTitlebarButton
    // shift branch and restore `static let hostWidth: CGFloat = 164` when both the
    // reload-button patch and the cluster-filter patch are removed.
    static let hostWidthTrailingReserve: CGFloat = 36
    static var hostWidth: CGFloat {
        computedHostWidth(config: TitlebarControlsStyle.classic.config)
    }
    static func computedHostWidth(config: TitlebarControlsStyleConfig) -> CGFloat {
...
enum MinimalModeSidebarControlActionSlot: Int, CaseIterable {
    ...
    // CASPER: Casper shows only the New Tab button in the sidebar-top cluster;
    // the other four are hidden so the cluster doesn't desync the AppKit
    // hit-region overlay. Delete and replace with `allCases` if upstream adds
    // per-slot gating or Casper adopts the full cluster.
    static var visibleSlots: [MinimalModeSidebarControlActionSlot] {
        CasperBuildEnvironment.isBranded ? [.newTab] : Array(allCases)
    }
