...
func titlebarHintLayoutRightmostExtent(
    ...
    // CASPER: mirror titlebarHintIntervals' reloadButtonShift so the reserved
    // accessory width accounts for the extra slot button when CasperReloadTitlebarButton
    // is rendered. Without this the rightmost hint pill clips. Delete with the
    // reload-button patch.
    let reloadButtonShift = TitlebarControlsLayoutMetrics.casperReloadButtonShift(config: config)
    // CASPER: filter to visible slots so extent calculation only covers rendered
    // buttons. hintInterval uses visible-position x-coords (same as
    // titlebarHintIntervals). Delete filter and restore allCases when
    // cluster-filter patch is removed.
    let visibleSlots = MinimalModeSidebarControlActionSlot.visibleSlots
    let hintSlots = TitlebarShortcutHintActionSlot.allCases.filter {
        visibleSlots.contains($0.controlSlot)
    }
    ...
    for slot in hintSlots {
        ...
        var interval = TitlebarControlsLayoutMetrics.hintInterval(
            for: slot,
            width: width,
            config: config,
            xOffset: xOffset
        )
        if reloadButtonShift > 0 {
            interval = (interval.lowerBound + reloadButtonShift)...(interval.upperBound + reloadButtonShift)
        }
...
enum TitlebarShortcutHintActionSlot: Int, CaseIterable {
    ...
    /// Corresponding action slot in the button cluster; used to determine
    /// visible position when Casper's cluster-filter patch is active.
    var controlSlot: MinimalModeSidebarControlActionSlot {
        switch self {
        case .toggleSidebar: return .toggleSidebar
        case .showNotifications: return .showNotifications
        case .newTab: return .newTab
        case .focusHistoryBack: return .focusHistoryBack
        case .focusHistoryForward: return .focusHistoryForward
        }
    }
}
...
enum TitlebarControlsLayoutMetrics {
    ...
    // CASPER: extra leading offset applied to every hint interval when the DEBUG
    // reload button occupies the leading slot. Both titlebarHintIntervals and
    // titlebarHintLayoutRightmostExtent MUST use this so reserved accessory width
    // matches rendered hint positions. Delete with the reload-button patch.
    static func casperReloadButtonShift(config: TitlebarControlsStyleConfig) -> CGFloat {
        #if DEBUG
        return CasperReloadTitlebarButton.isRendered ? config.buttonSize + config.spacing : 0
        #else
        return 0
        #endif
    }
    ...
    static func buttonRowWidth(config: TitlebarControlsStyleConfig) -> CGFloat {
        // CASPER: count only the visible slots so contentSize doesn't reserve
        // space for hidden buttons. Stock builds still count all five. Delete
        // and restore `TitlebarShortcutHintActionSlot.allCases.count` if
        // cluster-filter patch is removed.
        let visibleHintSlots = TitlebarShortcutHintActionSlot.allCases.filter {
            MinimalModeSidebarControlActionSlot.visibleSlots.contains($0.controlSlot)
        }
        let buttonCount = CGFloat(visibleHintSlots.count)
        let gapCount = max(0, buttonCount - 1)
        return (buttonCount * config.buttonSize) + (gapCount * config.spacing)
    }

    // CASPER: buttonCenterX now accepts a visible index (position within
    // visibleSlots) rather than the slot's allCases rawValue, so hint pills sit
    // over the correct rendered button. Delete and restore `slot.rawValue` cast
    // if cluster-filter patch is removed.
    static func buttonCenterX(
        atVisibleIndex visibleIndex: Int,
        config: TitlebarControlsStyleConfig
    ) -> CGFloat {
        let index = CGFloat(visibleIndex)
    ...
    static func hintInterval(
        ...
        // CASPER: derive x-position from slot's visible index so each hint pill
        // sits over its button after the cluster-filter removes hidden slots. Falls
        // back to allCases index for non-visible slots (shouldn't happen because
        // callers filter by visibleSlots first). Delete visibleIndex lookup and
        // restore `buttonCenterX(for: slot, …)` call when cluster-filter is removed.
        let visibleSlots = MinimalModeSidebarControlActionSlot.visibleSlots
        let visibleIndex = visibleSlots.firstIndex(of: slot.controlSlot) ?? slot.rawValue
        let centerX = buttonCenterX(atVisibleIndex: visibleIndex, config: config) + xOffset
...
struct TitlebarControlsView: View {
    ...
    private func controlsGroup(config: TitlebarControlsStyleConfig, foregroundColor: Color) -> some View {
        ...
        // CASPER: visibleSlots is the single source of truth for which buttons
        // render. Stock builds include all five; Casper builds include only newTab.
        // The `if visibleSlots.contains` guards below keep @ViewBuilder syntax valid
        // (for/guard are illegal in result-builder bodies). Delete visibleSlots
        // capture and all guards if upstream adds per-slot gating or Casper adopts
        // the full cluster.
        let visibleSlots = MinimalModeSidebarControlActionSlot.visibleSlots
        let content = HStack(spacing: config.spacing) {
            ...
            CasperReloadTitlebarButton(config: config, foregroundColor: foregroundColor)
            #endif

            if visibleSlots.contains(.toggleSidebar) {
            ...
            if visibleSlots.contains(.showNotifications) {
                TitlebarControlButton(
                    config: config,
                    foregroundColor: foregroundColor,
            ...
            if visibleSlots.contains(.newTab) {
                TitlebarControlButton(
                    config: config,
                    foregroundColor: foregroundColor,
            ...
            if visibleSlots.contains(.focusHistoryBack) {
                TitlebarControlButton(
                    config: config,
                    foregroundColor: foregroundColor,
            ...
            if visibleSlots.contains(.focusHistoryForward) {
                TitlebarControlButton(
                    config: config,
                    foregroundColor: foregroundColor,
    ...
    private func titlebarHintIntervals(
        ...
        // CASPER: `controlsGroup` renders `CasperReloadTitlebarButton` at the
        // leading position ahead of all slot buttons. The reload button shifts
        // every slot button one step right, so we must offset hint x-positions
        // by one button's width + spacing. Uses upstream's hintInterval baseline
        // and adds the Casper offset on top. Delete when upstream adds a generic
        // dev-reload affordance.
        let reloadButtonShift = TitlebarControlsLayoutMetrics.casperReloadButtonShift(config: config)
        // CASPER: filter to visible slots so hidden buttons don't produce hint
        // pills. hintInterval uses the slot's visible position (not rawValue)
        // so each pill sits over its rendered button. Delete filter and restore
        // allCases when cluster-filter patch is removed.
        let visibleSlots = MinimalModeSidebarControlActionSlot.visibleSlots
        let hintSlots = TitlebarShortcutHintActionSlot.allCases.filter {
            visibleSlots.contains($0.controlSlot)
        }

        return hintSlots.compactMap { slot in
            let shortcut = KeyboardShortcutSettings.shortcut(for: slot.action)
