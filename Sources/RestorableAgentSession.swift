...
enum AgentResumeCommandBuilder {
    ...
    // CASPER: fresh-launch fallback for orphaned Claude sessions — no transcript
    // found on disk, so resume would exit with "No conversation found". Falls
    // back to a new session with the original launch settings.
    // Delete if upstream adds its own orphan-detection / fresh-launch path.
    ...
    private static func agentMetadata(
        ...
        case .grok: return ("grok", "grok")
        case .pi: return ("pi", "pi")
        case .amp: return ("amp", "amp")
        case .kiro: return ("kiro-cli", "kiro")
...
struct SessionRestorableAgentSnapshot: Codable, Sendable {
    ...
    // CASPER: orphan-detection overload — when homeDirectory is provided and
    // the Claude transcript is missing on disk, falls back to a fresh launch
    // with a notice so the user knows why.
    // Delete if upstream adds its own orphan-detection / fresh-launch path.
    ...
    // CASPER: support freshLaunchStartupInput for orphan fallback.
    // Delete if upstream adds its own orphan-detection / fresh-launch path.
...
struct RestorableAgentSessionIndex: Sendable {
    ...
    // CASPER: transcript-exists check for orphan detection in resumeStartupInput(homeDirectory:).
    // Delete if upstream adds its own orphan-detection / fresh-launch path.
    static func claudeTranscriptExists(
        ...
        guard let workingDirectory, !workingDirectory.isEmpty else {
            // Without a cwd we can't verify; let Claude surface its own error.
            return true
        }
        let lookup = ClaudeTranscriptLookupCache(homeDirectory: homeDirectory, fileManager: fileManager)
        let fakeRecord = RestorableAgentHookSessionRecord(
            sessionId: sessionId,
            workspaceId: "",
            surfaceId: "",
        ...
        return claudeTranscriptExists(for: fakeRecord, fileManager: fileManager, lookup: lookup)
    }
