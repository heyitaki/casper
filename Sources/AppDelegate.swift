...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation, NSMenuDelegate {
    ...
    func createMainWindow(
        ...
        // CASPER: When the sidebar is collapsed in windowed minimal mode the
        // traffic lights now live in a dedicated 28pt titlebar strip above
        // the bonsplit tab bar (see ContentView.effectiveTitlebarPadding), so
        // the tab strip itself no longer reserves leading space for them —
        // seed the first-frame inset at 0.
    ...
    private func handleCustomShortcut(event: NSEvent) -> Bool {
        ...
        if matchConfiguredShortcut(event: event, action: .closeTab) {
            ...
            // group. The mode clears the moment the user touches a session,
            // after which ⌘W closes the focused panel again. Delete with the
