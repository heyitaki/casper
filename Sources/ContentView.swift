...
struct ContentView: View {
    ...
    @State private var activeResizerBandHandle: SidebarResizerHandle?
    @State private var resizerDragMovementExceededTapThreshold = false
    ...
    private func dividerBandContains(pointInContent point: NSPoint, contentBounds: NSRect) -> Bool {
        computeActiveResizerHandle(pointInContent: point, contentBounds: contentBounds) != nil
    }

    private func computeActiveResizerHandle(
        pointInContent point: NSPoint,
        contentBounds: NSRect
    ) -> SidebarResizerHandle? {
        guard point.y >= contentBounds.minY, point.y <= contentBounds.maxY else { return nil }
    ...
    private func resolvedSidebarResizerCursor() -> NSCursor {
        // Show the pointing-hand on hover so the band advertises its
        // click-to-collapse role (mirroring the collapsed-edge reveal strip).
        // Switch to the resize cursor as soon as the user actually drags so the
        // resize affordance is correct mid-gesture.
        if isResizerDragging, resizerDragMovementExceededTapThreshold {
            return Self.fixedSidebarResizeCursor
        }
        if let handle = activeResizerBandHandle, resizerHandleSupportsTap(handle), !isResizerDragging {
            return NSCursor.pointingHand
        }
        return Self.fixedSidebarResizeCursor
    }

    private func applySidebarResizerCursor() {
        resolvedSidebarResizerCursor().set()
    }
    ...
    private func updateSidebarResizerBandState(using _: NSEvent? = nil) {
        guard sidebarState.isVisible || rightSidebarVisible,
            ...
            if activeResizerBandHandle != nil {
                activeResizerBandHandle = nil
            }
        ...
        let activeHandle = computeActiveResizerHandle(
            pointInContent: pointInContent,
            contentBounds: contentView.bounds
        )
        let isInDividerBand = (activeHandle != nil)
        isResizerBandActive = isInDividerBand
        if activeResizerBandHandle != activeHandle {
            activeResizerBandHandle = activeHandle
        }
        ...
        if isInDividerBand || isResizerDragging {
            ...
            // Re-assert on the next runloop turn to keep the resolved cursor stable.
            ...
            .gesture(
                DragGesture(minimumDistance: 0, coordinateSpace: .global)
                    .onChanged { value in
                        ...
                        if !resizerDragMovementExceededTapThreshold,
                           movement >= SidebarRevealStripMetrics.tapThreshold {
                            resizerDragMovementExceededTapThreshold = true
                        }
                        activateSidebarResizerCursor()
                    ...
                    .onEnded { value in
                        ...
                        if let onTap,
                           abs(value.translation.width) < SidebarRevealStripMetrics.tapThreshold,
                           abs(value.translation.height) < SidebarRevealStripMetrics.tapThreshold {
                            onTap()
                        }
                        resizerDragMovementExceededTapThreshold = false
                        activateSidebarResizerCursor()
    ...
    private func placedSidebarResizerOverlay(
        ...
        GeometryReader { proxy in
            ...
            // Drive hairline visibility off the NSEvent-monitor-tracked band
            // state instead of SwiftUI .onHover. The portal NSView sits above
            // SwiftUI and can intermittently swallow tracking events near the
            // divider, which made the hover hairline flicker on the trailing
    ...
    /// Thin strip rendered along the leading edge when the sidebar is hidden.
    /// Click reopens the sidebar. The strip extends to the very top in both
    /// fullscreen and windowed modes; in windowed mode it overlaps the
    /// macOS resize hit zone, and the underlying NSView captures clicks
    ...
    private func terminalContent(appearance: WindowAppearanceSnapshot) -> some View {
        ...
        .overlay(alignment: .top) {
            // CASPER: Titlebar overlay is only over terminal content, not the sidebar.
            ...
            // Delete if upstream adds equivalent collapsed-sidebar titlebar handling.
    ...
    private func syncTrafficLightInset() {
        // CASPER: Traffic lights occupy a dedicated top-bar strip rendered above
        ...
        // longer needs to reserve leading space for them. Zero unconditionally.
        // Delete if upstream adds equivalent collapsed-sidebar traffic-light handling.
    ...
    /// Delete if upstream removes reveal strips or absorbs separator insets.
    ...
    var body: some View {
        ...
        view = AnyView(view.background(WindowAccessor(refreshID: appearance.appKitWindowMutationID) { [appearance] window in
            ...
            // CASPER: enforce isMovable=false in minimal mode so AppKit doesn't
            // auto-drag from the tab strip; explicit drag paths bracket performDrag.
            // Idempotent with configureCmuxMainWindowDragBehavior if upstream
            // already forces isMovable=false for the main window unconditionally.
            // Delete if upstream's function covers this fully.
...
// SidebarResizerAccessibilityModifier lives in Sources/Casper/Sidebar/SidebarRevealStrip.swift
// (same shape as upstream's; the reveal strip also uses it).
...
struct VerticalTabsSidebar: View {
    ...
    // CASPER: needed for workspaceSearchBarReservedHeight and sidebarWorkspaceListExtraTopOffset.
    ...
    // reads stay at the sidebar layer. Delete with the activity-state patch.
    ...
    var body: some View {
        // CASPER: explicit read so body re-evals when a throttled statusEntries
        // publish bumps the counter, keeping activity-sort ordering fresh.
        // Ordering updates need a body re-eval; TimelineView in
        // CasperWorkspaceActivityIndicator ticks the indicator itself but
        // doesn't reorder rows. Delete with the activity-state patch.
        ...
        ZStack(alignment: .bottomLeading) {
            ...
            // CASPER: Casper footer trims onSendFeedback/updateViewModel params
            // (single gear-button design); delete if upstream adopts the same footer.
            SidebarFooter()
                .frame(maxWidth: .infinity, alignment: .leading)
        }
        .overlay(alignment: .top) {
            // CASPER: workspace search bar pinned below titlebar icons.
            // Delete if upstream adds a workspace search field.
            workspaceSearchBar
        }
        .overlay {
        ...
        // CASPER: keep per-workspace statusEntries subscriptions aligned with
        // the visible set so the "3 blue dots" agent-activity indicator updates.
        // Delete with the activity-state patch.
        .onAppear {
    ...
    private func workspaceScrollArea(renderContext: WorkspaceListRenderContext) -> some View {
        ...
        // CASPER: compute the model once so scroll handlers use the same
        // filtered/sorted context as the visible rows. Without this, scroll-to-
        // selected silently no-ops while a search query is active (the outer
        // renderContext doesn't contain the filtered workspaceIds), and clearing
        // the query never retriggers the scroll because the onChange compares
        // the unfiltered ids. Delete with the activity-state patch.
        let model = casperWorkspaceRowsModel(renderContext: renderContext)
        return GeometryReader { geometryProxy in
            ScrollViewReader { scrollProxy in
                ...
                .overlay(alignment: .top) {
                    if isMinimalMode {
                        // CASPER: trailing-aligned HStack to avoid sideways jump across OS
                        // fullscreen transition. Delete if upstream adopts equivalent alignment.
                        HStack(spacing: 0) {
                            Spacer(minLength: 0)
                ...
                .onAppear {
                    // CASPER: use filtered context so scroll-to-selected works
                    // while a search query is active. Delete with activity-state patch.
                    requestSelectedWorkspaceScroll(scrollProxy, renderContext: model.renderContext)
                }
                .onChange(of: tabManager.selectedTabId) { _, _ in
                    requestSelectedWorkspaceScroll(scrollProxy, renderContext: model.renderContext)
                }
                // CASPER: observe filtered workspaceIds so clearing a search query
                // (which changes the filtered id set) retriggers the scroll request.
                // Delete with activity-state patch.
                .onChange(of: model.renderContext.workspaceIds) { oldWorkspaceIds, newWorkspaceIds in
                    guard shouldRequestSelectedWorkspaceScrollAfterWorkspaceIdsChange(
                        from: oldWorkspaceIds,
                        to: newWorkspaceIds
                    ) else {
                        flushPendingSelectedWorkspaceScroll(scrollProxy, renderContext: model.renderContext)
                        return
                    }
                    requestSelectedWorkspaceScroll(scrollProxy, renderContext: model.renderContext)
                }
    ...
    private func workspaceScrollContent(
        model: CasperWorkspaceRowsModel,
        minHeight: CGFloat
    ) -> some View {
        // CASPER: receives precomputed model so workspaceRows doesn't recompute
        // it a second time and scroll handlers share the same filtered context.
        // Delete with the activity-state patch.
        VStack(spacing: 0) {
            workspaceRows(model: model)
    ...
    // CASPER: imperative model assembly for `workspaceRows` (sort, filter,
    // JSONL session maps). Lives outside the @ViewBuilder because result
    // builders reject `for` loops. Delete with the activity-state patch.
    private struct CasperWorkspaceRowsModel {
        let renderContext: WorkspaceListRenderContext
        let activityByID: [UUID: CasperWorkspaceActivity]
        let claudeWorkspaceSessions: [UUID: [String]]
        let claudePanelSessions: [UUID: [String]]
        // CASPER: Codex-native session ids, resolved through the codex hook map.
        // Parallel to the Claude JSONL maps above so both history sources drive
        // the same 20s poll. Delete with the activity-state patch.
        let codexWorkspaceSessions: [UUID: [String]]
        let codexPanelSessions: [UUID: [String]]
        let claudeWorkspaceSessionsTaskID: CasperAgentSessionsTaskID
    ...
    private struct CasperBrandedPlan {
        ...
        /// CASPER: archived rows, rendered in the bottom Archive section rather
        /// than in the repo groups. Delete with the archive feature.
        let archivedPanelEntries: [CasperSidebarPanelEntry]
        /// CASPER: ⌘N digit per group key (1 = top group) — ⌘N selects a GROUP in
        /// branded builds, so the badge belongs on the header, not on a row.
        /// Delete with the group-selection feature.
        let groupShortcutDigitByKey: [String: Int]
        /// CASPER: distinct workspace ids per group in display order, for the
        /// group header's "Close All Sessions". Delete with the group-close feature.
        let groupWorkspaceIdsByKey: [String: [UUID]]
        /// Whether any group header renders (drives Archive-section top padding).
    ...
    private func casperWorkspaceRowsModel(renderContext: WorkspaceListRenderContext) -> CasperWorkspaceRowsModel {
        ...
        // CASPER: Group-contiguous unit sort. Upstream's renderItems run-detection
        // assumes group members are contiguous — sorting individual tabs breaks
        // collapsed-group suppression when activity interleaves members with
        // non-members. Instead: build units (one unit = consecutive members sharing
        // the same non-nil groupId, each ungrouped tab is its own unit), pick the
        // most-active member as that unit's representative for comparison, sort
        // units, then flatten. In-unit member order is preserved unchanged.
        // Delete with the activity-state patch when upstream adds agent state.
        typealias Unit = (members: [Workspace], offset: Int)
        var units: [Unit] = []
        var i = 0
        while i < filteredTabs.count {
            let tab = filteredTabs[i]
            if let gid = tab.groupId {
                var members = [tab]
                var j = i + 1
                while j < filteredTabs.count, filteredTabs[j].groupId == gid {
                    members.append(filteredTabs[j])
                    j += 1
                }
                units.append((members: members, offset: i))
                i = j
            } else {
                units.append((members: [tab], offset: i))
                i += 1
            }
        }
        // Precompute each unit's most-active member once; the sort comparator
        // runs O(U log U) times and must stay a pure dict lookup.
        let unitsWithReps: [(unit: Unit, rep: Workspace)] = units.map { unit in
            let rep = unit.members.max { a, b in
                CasperAgentActivity.compareActivityDesc(lhs: b, rhs: a, activityByID: activityByID)
            } ?? unit.members[0]
            return (unit: unit, rep: rep)
        }
        let sortedUnits = unitsWithReps.sorted { lhs, rhs in
            if CasperAgentActivity.compareActivityDesc(lhs: lhs.rep, rhs: rhs.rep, activityByID: activityByID) {
                return true
            }
            if CasperAgentActivity.compareActivityDesc(lhs: rhs.rep, rhs: lhs.rep, activityByID: activityByID) {
                return false
            }
            return lhs.unit.offset < rhs.unit.offset
        }
        let sortedTabs = sortedUnits.flatMap(\.unit.members)

        // Build JSONL session maps for the periodic off-main activity scan.
        ...
        for tab in filteredTabs {
            let paths = CasperAgentActivity.claudeJSONLPaths(for: tab, now: agentActivityNow)
        ...
        // Rebuild render context with sorted+filtered tab list so the upstream
        // render-item pipeline operates on Casper's activity-sorted order.
        let sortedWorkspaceById = Dictionary(uniqueKeysWithValues: sortedTabs.map { ($0.id, $0) })
        let casperRenderContext = WorkspaceListRenderContext(
            tabs: sortedTabs,
            tabIds: sortedTabs.map(\.id),
            sidebarReorderIds: renderContext.sidebarReorderIds,
            workspaceCount: renderContext.workspaceCount,
            canCloseWorkspace: renderContext.canCloseWorkspace,
            workspaceNumberShortcut: renderContext.workspaceNumberShortcut,
            tabItemSettings: renderContext.tabItemSettings,
            tabIndexById: renderContext.tabIndexById,
            workspaceById: sortedWorkspaceById,
            selectedContextTargetIds: renderContext.selectedContextTargetIds,
            selectedRemoteContextMenuWorkspaceIds: renderContext.selectedRemoteContextMenuWorkspaceIds,
            allSelectedRemoteContextMenuTargetsConnecting: renderContext.allSelectedRemoteContextMenuTargetsConnecting,
            allSelectedRemoteContextMenuTargetsDisconnected: renderContext.allSelectedRemoteContextMenuTargetsDisconnected,
            workspaceGroups: renderContext.workspaceGroups,
            workspaceGroupById: renderContext.workspaceGroupById,
        ...
        if CasperBuildEnvironment.isBranded {
            ...
            // Archive section, everything else feeds the repo-grouped plan.
            // Delete with the archive feature.
            let activePanelEntries = filteredPanelEntries.filter { !$0.isArchived }
            let archivedPanelEntries = filteredPanelEntries.filter { $0.isArchived }
            ...
            // CASPER: ⌘N selects the Nth GROUP in branded builds, so the digit is
            // assigned per group in display order and rendered on the group header.
            // Delete with the group-selection feature.
            var groupShortcutDigitByKey: [String: Int] = [:]
            for (offset, group) in groups.enumerated() {
                if let digit = WorkspaceShortcutMapper.digitForWorkspace(
                    at: offset,
                    workspaceCount: groups.count
                ) {
                    groupShortcutDigitByKey[group.key] = digit
                }
            }
            // Distinct workspace ids per group, display order — for "Close All".
            var groupWorkspaceIdsByKey: [String: [UUID]] = [:]
            for group in groups {
                var seen = Set<UUID>()
                groupWorkspaceIdsByKey[group.key] = group.entries.compactMap { entry in
            ...
            brandedPlan = CasperBrandedPlan(
                ...
                archivedPanelEntries: archivedPanelEntries,
                groupShortcutDigitByKey: groupShortcutDigitByKey,
                groupWorkspaceIdsByKey: groupWorkspaceIdsByKey,
                // Must mirror CasperSidebarRenderPlan.build's header gate exactly:
                // a single group renders no header, so the Archive section must not
                // add between-group padding above headerless rows.
                hasVisibleGroups: groups.count > 1
            )
        } else {
            brandedPlan = nil
        }
        ...
        return CasperWorkspaceRowsModel(
            ...
            codexWorkspaceSessions: codexWorkspaceSessions,
            codexPanelSessions: codexPanelSessions,
    ...
    @ViewBuilder
    private func workspaceRows(model: CasperWorkspaceRowsModel) -> some View {
    ...
    private func brandedWorkspaceRows(plan: CasperBrandedPlan, model: CasperWorkspaceRowsModel) -> some View {
        ...
        // pre-flat-plan group section's `.padding(.top, offset == 0 ? 0 : betweenGroupSpacing)`.
        ...
        let rows = LazyVStack(spacing: withinGroupSpacing) {
            ForEach(plan.items, id: \.id) { item in
                ...
                case .repoGroupHeader(let group, let isCollapsed, let isFirstInPlan):
                    ...
                    // A section wrapper (header + rows in one VStack) is incompatible
                    // with a flat ForEach, so CasperSidebarGroupHeaderRow renders the
                    // header alone and owns its own hover state.
                    CasperSidebarGroupHeaderRow(
                        group: group,
                        isCollapsed: isCollapsed,
                        // CASPER: ⌘N targets the group, so the badge lives here.
                        shortcutDigit: plan.groupShortcutDigitByKey[group.key],
                        shortcutModifierSymbol: casperRenderContext.workspaceNumberShortcut.numberedDigitHintPrefix,
                        ...
                        onCloseAll: { [weak tabManager] in
                            guard let ids = plan.groupWorkspaceIdsByKey[group.key] else { return }
                            tabManager?.casperCloseWorkspaceGroup(workspaceIds: ids)
                        }
                    )
                ...
                case .panelRow(let entry, let ownsWorkspaceAnchor):
                    ...
                    CasperSidebarPanelRow(
                        ...
                        // CASPER: per-row context menu (rename/pin/fork/archive/…).
                        // Built here where TabManager is in scope and handed to the
                        // row as value+closures (snapshot-boundary rule).
                        actions: casperSidebarRowActions(
                            for: entry,
                            workspaceLookup: casperRenderContext.workspaceById
                        )
                    )
                    .equatable()
            ...
            // Archived rows never own a workspace anchor — the anchor belongs to
            // the workspace's first ACTIVE row in the plan above, and a duplicate
            // .id(workspaceId) here would break scrollTo(selectedWorkspaceId).
            // Delete with the archive feature.
            if !plan.archivedPanelEntries.isEmpty {
                CasperArchiveSection(
                    count: plan.archivedPanelEntries.count,
                    isCollapsed: archiveStore.isCollapsed,
                    onToggle: { [weak archiveStore] in
                        archiveStore?.toggleCollapsed()
                    }
                ) {
                    ForEach(plan.archivedPanelEntries, id: \.id) { entry in
                        CasperSidebarPanelRow(
                            entry: entry,
                            isInMultiSelection: false,
                            onSelect: { [weak tabManager] in
                                tabManager?.focusTab(entry.key.workspaceId, surfaceId: entry.key.panelId)
                            },
                            onClose: { [weak tabManager] in
                                guard let tabManager,
                ...
                .padding(.top, plan.hasVisibleGroups ? betweenGroupSpacing : 0)
            }
        }
        .padding(.vertical, SidebarWorkspaceListMetrics.rowVerticalPadding)
        .frame(maxWidth: .infinity, alignment: .leading)
        // CASPER: agent-history activity poll; initial + 20s re-scan. Claude and
        // Codex share one task so both sources refresh on the same cadence.
        // Delete with activity-state patch.
        .task(id: model.claudeWorkspaceSessionsTaskID) {
            ...
            let codexWorkspaceSessions = model.codexWorkspaceSessions
            let codexPanelSessions = model.codexPanelSessions
    ...
    private func nonBrandedWorkspaceRows(model: CasperWorkspaceRowsModel) -> some View {
        ...
        let renderItems = SidebarWorkspaceRenderItem.renderItems(
            tabs: casperRenderContext.tabs,
            groupsById: casperRenderContext.workspaceGroupById
        )
            ...
            ForEach(renderItems, id: \.id) { item in
                ...
                case .workspace(let tab):
                    workspaceRow(
                        ...
                        renderContext: casperRenderContext,
                        shouldCollectWorkspaceDropTargets: shouldCollectWorkspaceDropTargets,
                        // CASPER: pass precomputed activity to compact row indicator.
                        // Delete with the activity-state patch.
                        casperActivity: activityByID[tab.id]
        ...
        .padding(.vertical, SidebarWorkspaceListMetrics.rowVerticalPadding)
        .frame(maxWidth: .infinity, alignment: .leading)
        // CASPER: agent-history activity poll task; initial scan + 20s re-scan.
        // `.task(id:)` restarts when a hook-map version or the visible workspace
        // set changes. Delete with the activity-state patch.
        .task(id: model.claudeWorkspaceSessionsTaskID) {
            let workspaceSessions = model.claudeWorkspaceSessions
            let panelSessions = model.claudePanelSessions
            let codexWorkspaceSessions = model.codexWorkspaceSessions
    ...
    private func workspaceRow(
        ...
        // CASPER: precomputed per-row activity from the sidebar parent.
        // Delete with the activity-state patch.
        casperActivity: CasperWorkspaceActivity
    ) -> some View {
        ...
        let row = TabItemView(
            ...
            // CASPER: precomputed activity from the sidebar parent; in == so
            // activity changes trigger re-render. Delete with compact-row patch.
            casperActivity: casperActivity
        )
        .equatable()
        .id(tab.id)
...
struct TabItemView: View, Equatable {
    ...
    nonisolated static func == (lhs: TabItemView, rhs: TabItemView) -> Bool {
        ...
        // CASPER: activity in == so row re-renders when agent state flips.
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 4) {
            HStack(spacing: isCasperCompact ? 5 : 8) {
                ...
                // and use regular weight in compact mode. Delete with compact-row patch.
                Text(isCasperCompact
                     ? CasperWorkspaceTitle.displayTitle(workspaceSnapshot.title)
                     : workspaceSnapshot.title)
                    .font(.system(size: scaledFontSize(12.5), weight: isCasperCompact ? .regular : titleFontWeight))
                ...
                // CASPER: right edge. Activity indicator in compact mode;
                // standard close button + shortcut hint in non-compact mode.
                // Delete `if isCasperCompact` branch with the compact-row patch.
                if isCasperCompact {
