// CASPER: Behavior + perf tests for CasperFindResultsView.apply. Guards the
// duplicate-emit short-circuit, the rapid-prefix-narrowing diff path, and the
// per-apply work budget that the Find sidebar relies on for smooth typing.
...
@MainActor
final class CasperFindResultsViewApplyTests: XCTestCase {
    ...
    func testDuplicateEmitIsNoOp() {
        let view = CasperFindResultsView()
    ...
    /// A `.matches` → `.limited` transition can ride the same results buffer
    /// (the pipeline switches terminal status without appending more hits when
    /// it sees an upstream cap). The status field participates in the
    /// short-circuit guard, so the second apply must run — otherwise any
    /// status-driven UI (e.g. a "showing limited results" affordance) would
    /// fall behind.
    func testStatusChangeForcesApply() {
        ...
        view.apply(settledSnapshot(query: "foo", results: sharedResults, status: .limited(sharedResults.count)))
    ...
    ///
    /// We build a fresh `CasperFindResultsView` inside the `measure {}` block
