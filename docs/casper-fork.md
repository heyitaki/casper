...
### Sidebar new-workspace directory menu + always-on cwd inheritance

Replaces the sidebar `+` action with one shared AppKit directory menu and makes all non-menu new-workspace actions inherit the selected workspace directory in Casper builds.

- Casper-only files:
  - `Sources/Casper/Sidebar/CasperRecentWorkspaceDirectoryStore.swift` persists a canonicalized, deduplicated 30-entry directory MRU in a Casper-namespaced `UserDefaults` key, prunes nonexistent directories when read, and supplies the seven displayed recents.
  - `Sources/Casper/Sidebar/CasperNewWorkspaceMenuController.swift` is the only menu builder. It owns the live search field, `~`-abbreviated folder rows, the existing `showOpenFolderPanel()` action, and trailing `cmux.json` actions.
  - `Sources/Casper/Sidebar/CasperWorkspaceDirectoryResolver.swift` resolves a selected-workspace directory with an unconditional home-directory fallback.
- Upstream files modified (all `// CASPER:`-marked):
  - `Sources/AppDelegate.swift` replaces the former configured-items-only context-menu builder with one call into `CasperNewWorkspaceMenuController`; both mouse buttons use this wrapper, recent folders and the folder picker share `openWorkspaceForExternalDirectory`, and configured actions continue through `executeConfiguredCmuxAction`. The wrapper takes an optional `anchorRect` (the hit button's rect) and refuses a windowless anchor, so callers fall back to their own action instead of the click being swallowed.
  - `Sources/Update/UpdateTitlebarAccessory.swift` exposes the existing AppKit context-menu overlay as the left-click anchor and routes the shared `TitlebarControlsView` plus action through the AppDelegate wrapper, covering the vertical sidebar, fullscreen controls, and titlebar accessory from one hook. It also renders the plus as `folder.badge.plus` in branded builds, vends the hit button's rect via `minimalModeSidebarControlActionSlotRect`, and routes the minimal-mode proxy's `.newTab` (the accessibility/AXPress path, which bypasses the window monitor) through the same wrapper. A missing anchor or unshown menu falls back to the original new-workspace action.
  - `Sources/WindowDecorationsController.swift` routes the minimal-mode AppKit twin's left click through the same wrapper, with the original new-workspace action as its fallback. Its existing right-click path in `MinimalModeSidebarControls.swift` already calls that wrapper. Two of its three `performMinimalModeSidebarControlAction` call sites (window monitor, `sendEvent`) pass no anchor and swallow the event before the SwiftUI button runs, so the anchor is resolved from the hit-region registry; the anchor those call sites *can* pass is framed at the window's leading edge and is only a last resort.
  - `Sources/WindowDragHandleView.swift` adds the registry's `minimalModeSidebarControlActionAnchor(forWindowPoint:in:)` and the `minimalModeSidebarControlActionSlotRect` protocol requirement, and routes both it and the existing slot lookup through one shared `resolveSidebarControlAction` so they cannot disagree about which button a click hit.
  - `Sources/Update/MinimalModeSidebarControls.swift` adds `TitlebarControlsHitRegions.buttonRect` (single source of truth shared by `layout()` and `sidebarActionSlotRect`) and `sidebarActionSlotRect`, the per-button rect used to anchor the menu under the button rather than under the whole cluster. Its `rightMouseDown` `.newTab` case passes that rect so right-click anchors under the button like left-click.
  - `Sources/ContentView.swift` gates the sidebar control cluster's trailing inset (10pt branded, stock keeps 4pt) and routes the browser-stack "New Tab" row — which invokes the same `onNewTab` — through the shared menu with a matching icon.
  - `Sources/TabManager.swift` replaces only the branded build's implicit-directory gate with `CasperWorkspaceDirectoryResolver` and records recents at `makeWorkspaceForCreation`, which excludes session restore. Detached-surface promotion deliberately does not record because it moves an existing surface rather than opening a directory. Stock upstream settings code and defaults remain unchanged.
  - `Resources/Localizable.xcstrings` adds the search placeholder, Recents header, and folder-picker label in English and Japanese.
  - `cmuxTests/CasperWorkspaceDirectoryTests.swift` covers MRU ordering, canonical dedupe, cap, persistence, nonexistent-path pruning, restore bypassing the creation-recording seam, selected-directory resolution, and home fallback.
  - `cmux.xcodeproj/project.pbxproj` registers all three Casper sources and the test file.
- Deletion condition: delete if upstream adds a searchable recent-directory workspace menu and makes selected-workspace directory inheritance unconditional. Remove the three Casper files, the titlebar/minimal-mode/AppDelegate hooks, the registry anchor lookup + `sidebarActionSlotRect` + their protocol requirement, the `folder.badge.plus` icon branches (restore `"plus"`), the trailing-inset gate (restore a flat `4`), the browser-stack New Tab hook, the `TabManager` resolver call and its `makeWorkspaceForCreation` recorder, the localized strings, and the tests.
...
- `Sources/WindowDecorationsController.swift`
  - Touched by the minimal-mode window-movable policy (one-line call to `CasperMinimalModeWindowMovable.apply` inside `apply(to:)`) and the minimal-mode plus-menu routing hook. Re-add if upstream rewrites either decorations or control dispatch.
...
- `Sources/Update/UpdateTitlebarAccessory.swift`, `Sources/Update/MinimalModeSidebarControls.swift`, `Sources/WindowChromeMetrics.swift`
  - Touched by patch 2 (trailing-aligned hidden controls, fullscreen breathing room, content-sized host). `UpdateTitlebarAccessory.swift` also owns the shared plus-menu anchor/action hook; `MinimalModeSidebarControls.swift` supplies the existing right-click wrapper route.
...
- `cmux.xcodeproj/project.pbxproj`
  - Touched by patches 3 (new PanelCloseTabContextMenu.swift entry, removed legacy files), 6 (ripgrep PBXShellScriptBuildPhase + Copy CLI), the archive patch (registers `CasperArchiveStore.swift`), and the new-workspace directory patch (three Casper sources plus one test). Conflicts here are mechanical but always require manual resolution.
- `Resources/Localizable.xcstrings`
