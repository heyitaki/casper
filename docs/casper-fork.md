...
### 15) Main-thread hang watchdog + hang-sample retention
...
- `Sources/AppDelegate.swift` — patches 2, 3, 8, 12, 15.
- `Sources/App/DebugLogging.swift` — patch 15. The entire `MainThreadHangWatchdog` type is Casper-only; upstream's file is just the `cmuxDebugLog` shim, so an upstream edit there conflicts against the whole class.
...
- **Bespoke sidebar `CasperForkSession` path** — upstream's restored/live snapshot gate and Fork Conversation destination dispatcher now power both Casper menus; the PID-liveness gate, disk lookup, custom unavailable alert, and single-new-workspace action were deleted.
